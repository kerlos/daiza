...
//   ・重心高さ ：台座接地面（画像下端）から測った重心の高さ。
//   左右それぞれについて計算する。
//
// 幾何は base.ts が確定した支持範囲（supportLeftMm / supportRightMm、mm 座標）を
// そのまま使い、台座計算と同じ基準で角度を出す。

import type { AnalysisParameters, BaseResult, Centroid, StabilityResult } from '@/model/types';
 ...
 *
 * 重心高さは base.ts と同一の式（figureHeightMm − 重心の mm-y）。mmPerPixel が
 * figureHeightMm / imageHeight であるため、画像下端＝台座接地面が実寸で
 * figureHeightMm に対応し、そこからの高さがこの差になる。
 *
...
export function computeStability(
  ...
  const { supportLeftMm, supportRightMm } = base;

  // 台座接地面（画像下端）から測った重心高さ。base.ts と同じ導出。
  const centroidHeightMm = params.figureHeightMm - centroid.mm.y;
