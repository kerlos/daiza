...
**一通り実装済み**（PNG読み込み → 解析 → 2D/3Dプレビュー → SVG / Illustrator(.ai) エクスポート）。残っているのは `docs/TODO.md` のとおり、(1) ブラウザ・Illustrator 実機での目視確認、(2) 超巨大画像向けの解析解像度上限（任意・既定OFF。実機で問題が出たら着手）、(3) 台座形状の拡張（角丸・円形・楕円・正多角形・任意形状。SPEC「台座形状」「台座 footprint」節に策定済み・**未実装**）、(4) 将来拡張のバックログ。実装を進める際は必ず `docs/SPEC.md` を正典（source of truth）、`docs/TODO.md` を残タスク表として参照すること。以下はSPECの要点と実装上の勘所を、コード全体を読まずに把握できるよう要約したもの。
...
src/
  ...
  analysis/       imageLoader, scale, contour, distance, centroid, slot, base,
                  stability, pipeline, analysis.worker        … 純粋ロジック
...
- **台座幅・台座奥行**：いずれもユーザー指定値をそのまま実寸とする（自動算出しない）。幅は支持多角形の考え方で「重心が支持範囲内」（必要幅 = `max(2 × |重心X − 差込口中心X|, 差込口幅)`）を、奥行は「スリットが台座に収まる」（必要奥行 = `板厚 + 2 × |前後オフセット|`）を検査し、満たさなければ台座計算不可。倒れにくさの余裕は転倒角で判断する（安全率パラメータは廃止）。
- **転倒角**：左右・前後それぞれ `θ = atan(支持端距離 / 重心高さ)`。前後は画像に写らないため、台座の奥行中心を原点とする1軸（正=前）で扱い、重心の奥行位置＝スリット中心（差込口の前後オフセット）とする。
- **曲線補完の例外**：差込部の肩（台座上面ライン上の4つの角）は加工寸法に直結する直角なので丸めない（`analysis/slot` の `slotJunctionCorners` → `utils/curve` の `sharpCorners`）。

### プレビューの3モード

解析表示 / 完成プレビュー / 3D。いずれも**表示のみの切替**であり、解析結果・パラメータ・エクスポートには一切影響しない。2Dグリッドは既定OFF（絵柄・オーバーレイの読みやすさを優先）、3Dの床グリッドは既定ON。
...
以下は例外でクラッシュさせず、**プレビュー前面のオーバーレイ**として表示する（列の上へ積むとエラーの出入りでビューワーの寸法が変わり、表示が跳ねるため）：PNG読み込み失敗 / 非対応画像 / 透明画像（アルファ閾値が高すぎて不透明画素が残らない場合を含む）/ スケール計算不可 / 差込口配置不可 / 台座計算不可。
...
複数差込口・金属スタンド・密度マップ・複数PNG同時計算など。解析ロジックをUIから切り離しておくことでこれらに対応する。台座形状（角丸・円形・楕円・正多角形・任意形状）は SPEC 策定済み・未実装（TODO §3）。
