...
- **傾け（転倒シミュレーション）**：「方位角 φ ＋ 傾き量 θ」で任意方位へ倒す（`render/tilt3d`）。**支点を footprint の bbox から取ってはならない**：円・楕円では斜め方向の支点が図形の外（bbox の角）に出て台座が床から浮く。支点は転倒角と同じ**凸包の支持関数**が定める支持直線（接触辺／接触点の接線）に取り、回転軸もその直線（斜めを向くのでオイラー角ではなくクォータニオン）。転倒判定もその方位の θ(d) で行う（左右・前後を別々に見ると最悪方位での転倒を見落とす）。
