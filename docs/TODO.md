...
完成品を立体で確認できる第3の表示モードを追加する（`docs/SPEC.md`「3Dプレビューモード」参照）。
表示のみの切替で解析・パラメータ・エクスポートには影響しない。Three.js + React Three Fiber を
dynamic import で読み込み、初期バンドルを増やさない。

- [x] 依存追加：`three` / `@react-three/fiber` / `@react-three/drei`（型含む）。
      3D関連は `React.lazy` + dynamic import で分離チャンク化し、初期ロードへの影響ゼロを
      `npm run build` のチャンクサイズで確認する（3D は独立チャンク `Preview3d-*.js` 983kB /
      gzip 267kB。初期チャンクは 401.01kB → 402.57kB で、増分はトグル UI と lazy ラッパのみ）
- [x] `render/scene3d.ts`（React非依存の純粋ロジック）：解析結果 → 3Dシーン用データへの変換。
      統合カットライン（曲線補完・肩の直角保持を含む）を mm 座標の押し出し用形状へ、
      台座 footprint + 貫通スリットを穴つき形状へ変換。原点 = 接地面上の台座中心、
      Y上正・Z前正（換算は `X = x_mm − 差込部中心X` / `Y = 接地面_mm − y_mm` の2式のみ。
      曲線→頂点列は `utils/curve` に追加した `closedCurvePolyline`。穴つき形状は
      `components/preview3d/geometry3d.ts` が three の `Shape`/`ExtrudeGeometry` へ写す）
- [x] テクスチャ生成：ビットマップから絵柄テクスチャ（長辺2048px上限へダウンスケール）と
      白版マスク（α をしきい値で2値化、不透明領域のみ）を生成。`ImageData` を React state に
      置かない（pixelStore の教訓を踏襲）（`render/texture3d.ts`。絵柄は**白版と合成して
      不透明化**し `alphaTest` で切り抜く：three の transmission は屈折の背景バッファへ
      不透明オブジェクトしか描かず、半透明のままだとアクリル越しに絵柄が消えるため。
      実物の「白版の上にインクが載る」構造とも一致する）
- [x] `components/preview3d/`：R3F の Canvas・アクリル板（押し出し + 裏面の絵柄/白版レイヤ）・
      台座（貫通スリット）・スタジオ風環境（床 + 接地影 + 環境光）・オービットカメラ
      （仰角制限・視点リセット）（環境マップは外部 HDRI を取らず `Lightformer` から生成＝
      完全クライアントサイドを維持。視点リセットは `CameraRig` が resetToken でのみ構図を当て、
      パラメータ変更ではカメラを動かさない）
- [x] アクリルマテリアル：`MeshPhysicalMaterial`（transmission）でリアル寄りの透明表現。
      板・台座で共有
- [x] 転倒シミュレーション（3D）：左右・前後の2本傾けスライダー。支持端エッジを支点に回転、
      可動域 = 転倒角 + 10°、転倒角超過で警告色、操作中のみ支点エッジと余裕を補助表示
      （支点は傾ける向き側の底辺エッジ。group を「支点へ移動 → 回転 → 戻す」の2段で入れ子）
- [x] 分解/組立アニメーション：ボタンで板の分離⇔差し込みをイージング付きで再生。
      傾き 0 へ戻してから再生する（`ExplodeGroup` が demand 描画下で自分の次フレームを
      `invalidate()` し続けて自走。到達したら要求をやめるので静止中は GPU を使わない）
- [x] 台座の半透明化トグル（スリット内のツメの収まり確認）
- [x] `Preview.tsx` への統合：表示操作コントロールに3D切替を追加（解析結果があるときのみ有効）。
      3D中はルーラー・2Dオーバーレイ・2Dシミュレーション・完成プレビューのトグルを無効化し、
      2Dへ戻したときズーム・パン状態を維持する（`useViewport` に `enabled` を足してホイール
      ズームの購読を止め、3D のオービットとイベントを奪い合わないようにした。変換 state は
      保持。2D stage はアンマウントせず `hidden` で隠す＝Canvas の描画内容を失わないため）
- [x] パフォーマンス：`frameloop="demand"`（操作・アニメ中のみ連続描画）、DPR上限2、
      ジオメトリ・テクスチャは解析結果/画像が変わったときのみ再構築（`useMemo`）。
      3000px級画像で快適に動作することを確認する（要ブラウザ実機確認）
- [x] `docs/SPEC.md` との整合・`README.md`・`CLAUDE.md` の更新
- [x] 合成マスクで検証（Node）：解析 → `buildScene3d` の座標系が
      「カットライン上端 Y = 指定フィギュア高さ（誤差 0.05mm）／ツメ底面 Y = 0（接地面）／
      台座上面ラインに肩4点（首部±20・ツメ±10）／ツメ幅が差込口幅を超えない／
      スリット開口 = 板厚・中心 Z = 前後オフセット」を満たすことを確認
- [ ] ブラウザ実機で確認：見た目（透明感・白版の見え方）・傾け・アニメーション・
      低スペック環境での動作

## 27. 床のグリッド・テクスチャ（3D）

3Dプレビューの床に実寸グリッドとテクスチャを持たせる（`docs/SPEC.md`「床（グリッド・テクスチャ）」参照）。
表示のみの機能で、解析・パラメータ・エクスポートには影響しない。

- [x] 木目サンプルのアセット化：`src/assets/textures/wood.png`（3Dチャンクからのみ参照するため、
      2D利用時はダウンロードされない。`npm run build` で `wood-*.png` の参照元が `Preview3d-*.js`
      だけであることを確認済み。既定はテクスチャなしなので、木目を選ぶまで取得もされない）
- [x] `render/texture3d.ts` に床タイルの生成（`buildFloorTexture`）：タイル1枚 = 200mm 角へ
      テクスチャ画像を cover で収め、実寸グリッド（10mm マス・50mm 強調線）を**同じタイルへ焼き込む**。
      グリッドを透明な面として床へ重ねないのは、three の transmission が屈折の背景バッファへ
      不透明オブジェクトしか描かず、アクリル板・台座越しに格子が消えてしまうため（印刷レイヤと同じ制約）
- [x] タイルの敷き詰め（`geometry3d.buildTiledTexture`）：`RepeatWrapping` + repeat = 床 / タイル
      （整数なので継ぎ目が原点の格子に揃う）。異方性フィルタはデバイス上限まで
- [x] `components/preview3d/Floor.tsx`：床メッシュ。map は**常に持たせる**（null ↔ テクスチャの
      差し替えでは three がシェーダを組み直さず＝ USE_MAP が切り替わらないため、無地のときも
      下地色で塗ったタイルを貼る）
- [x] `components/preview3d/useFloorTexture.ts`：テクスチャの出所（既定のなし / 木目 /
      アップロード画像）の状態。デコードは `createImageBitmap` でブラウザ内完結、失敗しても直前の床を保つ。
      差し替え時に前の `ImageBitmap` を close（effect のクリーンアップでは閉じない：StrictMode の
      二重マウントで**使用中の**画像を閉じてしまうため）
- [x] 3Dビューポート内の操作UI：グリッド表示トグル（既定 ON）と、テクスチャの
      「なし（既定）/ 木目 / 画像…」切替 + ファイル選択（左のパラメータパネルには置かない）
- [x] `docs/SPEC.md` との整合・`README.md`・`CLAUDE.md` の更新
- [ ] ブラウザ実機で確認：グリッドが原点から 10mm 刻みで並ぶこと・アクリル越しにも格子が見えること・
      アップロード画像が床に反映されること

## 28. グリッド表示（2D）

2Dプレビューに実寸(mm)グリッドをトグルで敷けるようにする（`docs/SPEC.md`「ルーラー」節
「グリッド（2D）」参照）。表示のみの機能で、解析・パラメータ・エクスポートには影響しない。

- [x] `components/Grid.tsx`：ルーラーと同じ目盛り列（`render/ruler` の `buildRulerTicks`）を
      再利用し、その位置へビューポートいっぱいの線を引く（目盛りロジックを共有するので、
      格子はルーラーの目盛りと必ず一致し、間隔の自動切替も同時に起きる）。主/副/原点軸で
      線の濃さを変える
- [x] `components/Preview.tsx`：グリッド表示の state（既定 OFF）と、表示操作コントロールの
      トグル（`Grid3x3`）。グリッドは stage より**先に**置いて背面へ敷く（絵柄・オーバーレイを
      覆わない）。3D 中はトグルごと出さない（3D 側に床グリッドのトグルがあり、無効化した
      ボタンを残すと二重に見えるため）
- [x] `docs/SPEC.md` との整合・`README.md` の更新
- [ ] ブラウザ実機で確認：格子がルーラーの目盛りと一致すること・ズーム/パンに追従すること・
      3Dへ切り替えて戻しても表示状態が保たれること
